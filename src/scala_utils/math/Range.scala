...
object Range {
  private case class CRange_int (min: Int, max: Int) extends Iterable_range [Int] {
    override def incl (elem: Int) = if (this (elem)) this else toSet + elem
    override def excl (elem: Int) = if (this (elem)) toSet - elem else this
    override def contains (elem: Int) = elem >= min && elem <= max
    override def iterator = (min to max).iterator
  }

  private case class CRange_float (min: Float, max: Float) extends Range [Float] {
    override def apply (v: Float) = v >= min && v <= max
  }

  private case class CRange_double (min: Double, max: Double) extends Range [Double] {
    override def apply (v: Double) = v >= min && v <= max
  }
  ...
  class Range_arith_ops2 [T] (r: Range [T])(implicit ar: Arithmetic [T], rf: Range_factory [T]) {
    ...
    def & (r: Range [T]) = rf (ar.max_elems (r.min, r.min), ar.min_elems (r.max, r.max))
    def | (r2: Range [T]) = rf (ar.min_elems (r.min, r2.min), ar.max_elems (r.max, r2.max))
  }
...
object Range_float {
  ...
  val one = Range (1f, 1f)
  val zero_one = Range (0f, 1f)
}

object Range_double {
  val zero = Range (0.0, 0.0)
  val one = Range (1.0, 1.0)
  val zero_one = Range (0.0, 1.0)
}
